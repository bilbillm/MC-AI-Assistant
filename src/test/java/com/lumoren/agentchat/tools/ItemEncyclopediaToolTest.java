...
/**
 * Unit tests for {@link ItemEncyclopediaTool}.
 */
class ItemEncyclopediaToolTest {

    private final ItemEncyclopediaTool tool = new ItemEncyclopediaTool();

    @Test
    void getName_returnsCorrectName() {
        assertEquals("item_info", tool.getName());
    }
    ...
    void getDefinition_returnsValidDefinition() {
        ...
        assertEquals("item_info", def.name());
    ...
    void execute_withMockMinecraft_doesNotThrow() {
        ...
        try {
            ...
            assertEquals("item_info", result.toolCallId());
        } catch (Exception e) {
